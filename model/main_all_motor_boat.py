...
# --------- Motor/Boat posterior & gate feature columns (per-lane) ----------
# These are row-level columns so that we can inspect, export and run PI easily.
# NOTE: Global Z-score (relative to the whole pool) is still computed inside
#       BoatRaceDataset to avoid train/val leakage.
ALPHA_MTR, BETA_MTR = 3.0, 15.0   # prior mean ≈ 1/6
ALPHA_BOT, BETA_BOT = 3.0, 15.0
K_MTR, K_BOT       = 50.0, 50.0   # evidence gate strength (exchange‑fresh suppression)

# --- toggle: drop ALL boat equipment features (post/gate/var + raw deps)
DISABLE_BOAT_EQUIP = True

def _safe_col(df: pd.DataFrame, name: str, fill: float = 0.0) -> pd.Series:
    return df[name] if name in df.columns else pd.Series(fill, index=df.index)

def _mk_motor_post(lane: int):
    def _fn(df: pd.DataFrame) -> pd.Series:
        s = _safe_col(df, f"lane{lane}_motor_starts")
        f = _safe_col(df, f"lane{lane}_motor_firsts")
        return ((f + ALPHA_MTR) / (s + ALPHA_MTR + BETA_MTR)).astype("float32")
    return _fn

def _mk_boat_post(lane: int):
    def _fn(df: pd.DataFrame) -> pd.Series:
        s = _safe_col(df, f"lane{lane}_boat_starts")
        f = _safe_col(df, f"lane{lane}_boat_firsts")
        return ((f + ALPHA_BOT) / (s + ALPHA_BOT + BETA_BOT)).astype("float32")
    return _fn

def _mk_motor_gate(lane: int):
    def _fn(df: pd.DataFrame) -> pd.Series:
        s = _safe_col(df, f"lane{lane}_motor_starts")
        return (s / (s + K_MTR)).astype("float32")
    return _fn

def _mk_boat_gate(lane: int):
    def _fn(df: pd.DataFrame) -> pd.Series:
        s = _safe_col(df, f"lane{lane}_boat_starts")
        return (s / (s + K_BOT)).astype("float32")
    return _fn


for _l in range(1, 7):
    register_feature(FeatureDef(f"lane{_l}_motor_post", _mk_motor_post(_l),
                                deps=[f"lane{_l}_motor_starts", f"lane{_l}_motor_firsts"], dtype="float32"))
    register_feature(FeatureDef(f"lane{_l}_boat_post",  _mk_boat_post(_l),
                                deps=[f"lane{_l}_boat_starts",  f"lane{_l}_boat_firsts"],  dtype="float32"))
    register_feature(FeatureDef(f"lane{_l}_motor_gate", _mk_motor_gate(_l),
                                deps=[f"lane{_l}_motor_starts"], dtype="float32"))
    register_feature(FeatureDef(f"lane{_l}_boat_gate",  _mk_boat_gate(_l),
                                deps=[f"lane{_l}_boat_starts"],  dtype="float32"))

# --------- ADD: posterior variance & lightweight K/αβ tuning helpers ---------
def _mk_motor_post_var(lane: int):
    def _fn(df: pd.DataFrame) -> pd.Series:
        s = _safe_col(df, f"lane{lane}_motor_starts").astype("float32")
        f = _safe_col(df, f"lane{lane}_motor_firsts").astype("float32")
        # Posterior Beta(a', b') where a' = f + α, b' = (s - f) + β
        a = f + float(ALPHA_MTR)
        b = (s - f).clip(lower=0.0) + float(BETA_MTR)
        denom = (a + b) ** 2 * (a + b + 1.0)
        var = (a * b) / denom.replace(0.0, np.nan)
        return var.fillna(0.0).astype("float32")
    return _fn

def _mk_boat_post_var(lane: int):
    def _fn(df: pd.DataFrame) -> pd.Series:
        s = _safe_col(df, f"lane{lane}_boat_starts").astype("float32")
        f = _safe_col(df, f"lane{lane}_boat_firsts").astype("float32")
        a = f + float(ALPHA_BOT)
        b = (s - f).clip(lower=0.0) + float(BETA_BOT)
        denom = (a + b) ** 2 * (a + b + 1.0)
        var = (a * b) / denom.replace(0.0, np.nan)
        return var.fillna(0.0).astype("float32")
    return _fn
...
for _l in range(1, 7):
    register_feature(FeatureDef(f"lane{_l}_motor_post_var", _mk_motor_post_var(_l),
                                deps=[f"lane{_l}_motor_starts", f"lane{_l}_motor_firsts"], dtype="float32"))
    register_feature(FeatureDef(f"lane{_l}_boat_post_var",  _mk_boat_post_var(_l),
                                deps=[f"lane{_l}_boat_starts",  f"lane{_l}_boat_firsts"],  dtype="float32"))

# ---- Prior setter & lightweight proxy tuner ---------------------------------
def _set_equipment_prior(K_mtr: float = None, K_bot: float = None):
    """
    Update global K and (α,β) for motor/boat. Prior mean is fixed to 1/6; strength = K.
    If an argument is None, keep the current value.
    """
    global K_MTR, K_BOT, ALPHA_MTR, BETA_MTR, ALPHA_BOT, BETA_BOT
    if K_mtr is not None:
        K_MTR = float(K_mtr)
        # mean = 1/6 → α = K/6, β = 5K/6
        ALPHA_MTR = K_MTR / 6.0
        BETA_MTR  = K_MTR * 5.0 / 6.0
    if K_bot is not None:
        K_BOT = float(K_bot)
        ALPHA_BOT = K_BOT / 6.0
        BETA_BOT  = K_BOT * 5.0 / 6.0

def _spearman_proxy_score(df_feat: pd.DataFrame, kind: str) -> float:
    """
    Proxy: average Spearman corr across lanes between 'post' and -rank (1 is best).
    kind ∈ {'motor','boat'}
    """
    import numpy as _np
    cols = []
    for l in range(1, 7):
        post = f"lane{l}_{kind}_post"
        rank = f"lane{l}_rank"
        if post in df_feat.columns and rank in df_feat.columns:
            cols.append((post, rank))
    if not cols:
        return 0.0
    # rank_target: 低いほど良い → corr(post, -rank)
    cors = []
    for post, rank in cols:
        x = df_feat[post]
        y = -df_feat[rank]
        if x.notna().any() and y.notna().any():
            ...
            if pd.notna(c):
                cors.append(float(c))
    return float(_np.mean(cors)) if cors else 0.0

def tune_equipment_priors_proxy(df_raw: pd.DataFrame,
                                k_candidates=(5, 10, 20, 35, 50),
                                sample_rows: int = 120000) -> tuple[float, float]:
    """
    Lightweight, training‑free K selection by Spearman proxy.
    Separately picks K_mtr and K_bot that maximize the proxy on a time‑ordered sample.
    """
    # Take an early time‑ordered slice to respect causality a bit
    df = df_raw.sort_values("race_date")
    ...
    for K in k_candidates:
        _set_equipment_prior(K_mtr=K)
        tmp = apply_features(df, include=[f"lane{i}_motor_post" for i in range(1,7)], inplace=False)
        s = _spearman_proxy_score(tmp, kind="motor")
        if s > best_s:
            best_s, best_mtr = s, K
    ...
    best_bot = K_BOT
    best_s   = -1e9
    if not DISABLE_BOAT_EQUIP:
        for K in k_candidates:
            _set_equipment_prior(K_bot=K)
            tmp = apply_features(df, include=[f"lane{i}_boat_post" for i in range(1,7)], inplace=False)
            s = _spearman_proxy_score(tmp, kind="boat")
            if s > best_s:
                best_s, best_bot = s, K
    else:
        # keep existing K_BOT; boat features are disabled downstream anyway
        pass

    # finalize
    _set_equipment_prior(K_mtr=best_mtr, K_bot=best_bot)
    print(f"[tune] Chosen priors by proxy: K_mtr={best_mtr:.0f}, K_bot={best_bot:.0f} "
          f"(α_mtr={ALPHA_MTR:.2f}, β_mtr={BETA_MTR:.2f}; α_bot={ALPHA_BOT:.2f}, β_bot={BETA_BOT:.2f})")
    return best_mtr, best_bot
# --------- END: posterior variance & tuning helpers --------------------------
...
# --- Equipment prior sweep (proxy-based; fast) -------------------
try:
    DO_PRIOR_SWEEP = True
except NameError:
    DO_PRIOR_SWEEP = True
...
if DO_PRIOR_SWEEP:
    # Tune K (and α,β) with a training‑free Spearman proxy on an early slice
    tune_equipment_priors_proxy(result_df, k_candidates=(5, 10, 20, 35, 50), sample_rows=120000)
else:
    # Keep defaults; but ensure α,β reflect K with mean=1/6
    _set_equipment_prior(K_mtr=K_MTR, K_bot=K_BOT)

# --- 追加特徴量（Feature Registry 経由） ---
_feat_exclude = []
if 'DISABLE_BOAT_EQUIP' in globals() and DISABLE_BOAT_EQUIP:
    _feat_exclude = [f"lane{i}_{n}" for i in range(1,7) for n in ("boat_post","boat_gate","boat_post_var")]
result_df = apply_features(result_df, exclude=_feat_exclude)
...
# --- drop all BOAT equipment features from dataframes as well ---
if DISABLE_BOAT_EQUIP:
    for lane in range(1, 7):
        exclude.extend([
            f"lane{lane}_boat_post",
            f"lane{lane}_boat_gate",
            f"lane{lane}_boat_post_var",
            f"lane{lane}_boat_starts",
            f"lane{lane}_boat_firsts",
        ])
