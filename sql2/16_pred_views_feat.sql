...
SELECT DISTINCT ON (b.race_key, b.lane)
       ...
       w.weather_txt,
       w.wind_dir_deg
FROM core.pred_boat_info b
JOIN core.pred_weather  w  USING (race_key)
ORDER BY b.race_key, b.lane
WITH NO DATA;

-- Indexes for pred.boat_flat
CREATE UNIQUE INDEX IF NOT EXISTS uq_boat_flat_race_key_lane
  ON pred.boat_flat (race_key, lane);
CREATE INDEX IF NOT EXISTS idx_boat_flat_racer_id
  ON pred.boat_flat (racer_id);
...
CREATE INDEX IF NOT EXISTS idx_feat_tf2_lane1
  ON pred.features (lane1_racer_id, lane1_bf_course);
CREATE INDEX IF NOT EXISTS idx_feat_tf2_lane2
  ON pred.features (lane2_racer_id, lane2_bf_course);
CREATE INDEX IF NOT EXISTS idx_feat_tf2_lane3
  ON pred.features (lane3_racer_id, lane3_bf_course);
CREATE INDEX IF NOT EXISTS idx_feat_tf2_lane4
  ON pred.features (lane4_racer_id, lane4_bf_course);
CREATE INDEX IF NOT EXISTS idx_feat_tf2_lane5
  ON pred.features (lane5_racer_id, lane5_bf_course);
CREATE INDEX IF NOT EXISTS idx_feat_tf2_lane6
  ON pred.features (lane6_racer_id, lane6_bf_course);
